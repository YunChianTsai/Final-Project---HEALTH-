...
<div class="personal" style="background-color:#fff;border-radius: 20px;width: 800px;height: 100%;margin-top: -180px;margin-left: -400px;">
    <div class="title">
        <font size="6" color="#36648b" face="圓體"><b><?php echo $uname; ?>的飲食計畫</b></font>
    </div>
    <div class="personal_1">
        <a href='choose_d.php' style="color: #104e8b"><span class="fa fa-plus-circle" style="font-size: 2.5em"></span></a>
    </div>
    <div class="schedule" style="border-top:dotted;width:780px;border-top-color:#36648b;">
    <table style="width: 100%;height: 300px;">
    <?php
    echo "<tr>";
    echo "<td height='50px' align='center'><h5><b>食物名稱</td><td align='center'><h5><b>單位</td><td align='center'><h5><b>熱量</td></h5>";
    echo "</tr>";
    if(isset($_POST['diet'])){
        $cdiet = $_POST['diet'];
    }else{
    ...
    if($cdiet!=''){
        $i=0;
        $tkcal=0;
        for($i=0;$i<count($cdiet);$i++){
            $diet=$cdiet[$i];
            $SQL = "SELECT * FROM dietlist WHERE dname ='$diet'";
            $result = mysqli_query($link, $SQL);
            $d=array();
            $x=0;
            $y=0;
            while($row = mysqli_fetch_array($result)){
                $d[$x] = $row;
                echo "<tr>";
                echo "<td align='center'>".$d[$x][$y+1]."</td><td align='center'>".$d[$x][$y+2]."</td><td align='center'>".$d[$x][$y+3]." kcal"."</td>";
                echo "</tr>";
                $tkcal = $tkcal + $d[$x][$y+3] *1;
                $total = $tkcal.'kcal';
                $x++;
                $y++;
            }
    ...
    <tr style="border-top:dotted;width:780px;border-top-color:#36648b">
        <td colspan='3' align='center'><font>今日飲食攝取熱量：</font><?php echo $total; ?></td>
    </tr>
    <?php 
        if($sex == 1 && $height != 0 && $weight != 0 && $activity !=0){
            ...
            $tdee= ((10 * $weight) + (6.25 * $height) - (5 * $age) + (-161)) * $activity;
        }else{
            $tdee = '尚未完成基本資料填寫';
        }
        ?>
    <!-- <tr>
        <td colspan='3' align='center'><font>你的每日總消耗熱量(TDEE)：</font><?php ?></td>
    </tr> -->
    </table>
    </div>
</div>
